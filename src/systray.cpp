...
void Systray::createActions()
{
    ...
    //add by wallace young 
    showUserListAction = new QAction(tr("User List"), 0);
}
...
void Systray::createMenus()
{
    ...
    trayIconMenu->addAction(showUserListAction);
...
#define VER 1
#if VER == 0
void Systray::showUser()
{
    Global::userManager->display();
...
{
    Global::userManager->display();
    MainListWindow *pmlw = new MainListWindow();    
    pmlw->show();
}
#else
void Systray::showUser()
{
    ChatWindow *pcw = new ChatWindow();    
    pcw->show();
}
#endif
...
void Systray::createConnections()
{
    ...
    connect(showUserListAction, SIGNAL(triggered()),
            this, SLOT(showUser()));
}
